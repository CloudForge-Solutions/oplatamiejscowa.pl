...
const StaysManager: React.FC<StaysManagerProps> = ({
  ...
  const groupReservationsByCity = () => {
    ...
    // Group by city (using accommodationName as city proxy, fallback to guest country)
    const cityMap = new Map<string, BookingReservation[]>();

    filteredReservations.forEach(reservation => {
      // Extract city from accommodation name, guest country, or use a default
      let cityKey = reservation.accommodationName;

      if (!cityKey || cityKey.trim() === '') {
        // Fallback to guest country if no accommodation name
        cityKey = reservation.guestCountry || 'Unknown Location';
      }
