...
export const TouristTaxProvider: React.FC<TouristTaxProviderProps> = ({ children }) => {
  ...
  const loadReservation = async (id: string): Promise<void> => {
    ...
    try {
      ...
      logger.error('❌ Failed to load reservation', {
        error: errorMessage,
        reservationId: id
      });
  ...
  const initiatePayment = async (): Promise<void> => {
    ...
    try {
      ...
      // Create payment via mockup API
      const paymentResponse = await mockupApiService.createPayment({
        reservationId: reservation.id,
        amount: reservation.totalTaxAmount,
        currency: reservation.currency,
        returnUrl: `${window.location.origin}/payment/${reservation.id}/result`,
        notificationUrl: `${window.location.origin}/api/payment/webhook`
      });

      if (!paymentResponse.success) {
        throw new Error(paymentResponse.message || paymentResponse.error || 'Failed to create payment');
      }
      ...
      const imojePayment = await imojePaymentService.createPayment({
        amount: reservation.totalTaxAmount,
        currency: reservation.currency,
      ...
      storageService.set(STORAGE_KEYS.TOURIST_TAX_SESSION, {
        paymentStatus,
        reservationId: reservation.id,
        paymentUrl: imojePayment.paymentUrl
      });
      ...
      setTimeout(() => {
        window.location.href = imojePayment.paymentUrl;
      }, 1000);
