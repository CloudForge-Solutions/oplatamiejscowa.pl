...
def analyze_plans():
    ...
    try:
        ...
        if 'GitHub Pages' in content and 'Azure Functions' in content:
            issues.append('❌ CRITICAL: GitHub Pages cannot host Azure Functions backend')
        
        if 'SASTokenService.ts' in content and 'src/services/' in content:
        ...
        if 'Static React' in content and 'GitHub Pages' in content and 'Azure Functions' in content:
            issues.append('🚀 DEPLOYMENT: Static hosting + serverless functions need separate deployment')
        
        # Check for CORS issues
        if 'Direct Blob Access' in content and 'browser' in content.lower():
            issues.append('🌐 CORS: Direct blob access from browser requires CORS configuration')
