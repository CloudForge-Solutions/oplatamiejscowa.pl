...
const BookingReservationImport: React.FC<BookingReservationImportProps> = ({
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            try {
              ...
              const reservation: BookingReservation = {
                ...
                guestPhone: row[26] || '', // Column 26: Phone number
                ...
                numberOfNights: parsePositiveInteger(row[23], 0), // Column 23: Duration (nights)
                accommodationName: row[1] || '', // Column 1: Property name (Zarezerwowa B(a))
                accommodationAddress: row[25] || '', // Column 25: Address
                status: parseBookingStatus(row[6]), // Column 6: Status
                totalPrice: parseFloat(row[12]) || 0, // Column 12: Price
                commission: parseFloat(row[14]) || 0, // Column 14: Commission amount
                paymentStatus: row[15] || 'pending', // Column 15: Payment status
                bookingDate: parseBookingDate(row[5]), // Column 5: Booking Date
                specialRequests: row[17] || '', // Column 17: Comments
                taxStatus: 'pending'
              };
  ...
  const importSelected = async () => {
    ...
    try {
      ...
      await reservationRepository.saveMany(selectedReservationData);
      ...
      // Call the parent callback with selected reservations
