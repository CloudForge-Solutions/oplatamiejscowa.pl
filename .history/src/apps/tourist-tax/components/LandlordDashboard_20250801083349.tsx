...
const LandlordDashboard: React.FC = () => {
  ...
  const handleReservationsImported = async (newReservations: BookingReservation[]) => {
    // Reservations are already saved to IndexedDB by the import component
    // Just reload the data to update the dashboard
    await loadReservationsFromDB();

    // Switch to stays tab
    setActiveTab('stays');

    console.log(`Imported ${newReservations.length} new reservations`);
  };
    ...
    <Container fluid className="py-4">
      ...
      <Tab.Container activeKey={activeTab} onSelect={(k) => setActiveTab(k || 'overview')}>
        ...
        <Tab.Content>
          ...
          {/* Stays Tab (Pobyty) - Unified Reservations and Payments */}
          <Tab.Pane eventKey="stays">
            <StaysManager
              onStaySelect={(reservation) => {
                // Could implement stay details modal here
                console.log('Selected stay:', reservation);
              }}
              showActions={true}
            />
          </Tab.Pane>
