...
export class StorageService {
  ...
  async storeTransaction(
    ...
    options?: TransactionStorageOptions
  ): Promise<void> {
    if (!this.isInitialized) {
  ...
  async getTransactionByTransactionId(transactionId: string): Promise<DBTransaction | null> {
    ...
    if (this.capabilities.indexedDB) {
      try {
        const transaction =
          await this.indexedDBManager.getTransactionByTransactionId(transactionId);
        if (transaction) {
          return transaction;
        }
      } catch (error) {
  ...
  async performCleanup(olderThanDays = 365): Promise<{ deletedTransactions: number }> {
    ...
    if (this.capabilities.localStorage) {
      ...
      const filteredTransactions = transactions.filter(t => new Date(t.createdAt) > cutoffDate);

      if (filteredTransactions.length !== transactions.length) {
        localStorage.setItem(
          'tourist-tax-recent-transactions',
