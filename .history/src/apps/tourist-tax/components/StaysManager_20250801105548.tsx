...
const StaysManager: React.FC<StaysManagerProps> = ({
  ...
  const groupReservationsByCity = () => {
    ...
    filteredReservations.forEach(reservation => {
      // Extract city from accommodation address or name
      let cityKey = 'Unknown City';

      // Try to extract city from accommodation address first
      if (reservation.accommodationAddress) {
  ...
  return (
    <div>
      ...
      <Alert variant="info" className="mb-3">
        ...
        StaysManager loaded - Reservations: {reservations.length}, City Groups: {cityGroups.length}, Selected: {selectedStays.size}
      </Alert>
            ...
            <div>
              ...
              {reservations.length === 0 && (
                <Badge bg="info" className="ms-2">
                  {t('stays.getStarted', 'Get Started')}
                </Badge>
              )}
            </div>
            <Button
              variant={reservations.length === 0 ? "primary" : "outline-primary"}
              size="sm"
              onClick={() => setShowImportSection(!showImportSection)}
              aria-expanded={showImportSection}
            >
              <i className={`bi bi-chevron-${showImportSection ? 'up' : 'down'} me-1`}></i>
              {showImportSection ? t('common:actions.close', 'Close') : t('common:actions.import', 'Import')}
            </Button>
          </div>
        </Card.Header>
          ...
          <div>
            <Card.Body>
              <div className="mb-3">
                <Alert variant="info" className="mb-3">
                  <i className="bi bi-info-circle me-2"></i>
                  {t('import.instructions', 'Upload your Booking.com reservation export file (.xlsx or .xls) to import stays and automatically calculate tourist tax amounts.')}
                </Alert>
              </div>
