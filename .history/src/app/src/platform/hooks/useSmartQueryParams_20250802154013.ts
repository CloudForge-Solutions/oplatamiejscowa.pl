...
export function useSmartQueryParams() {
    ...
    useEffect(() => {
        ...
        if (currentPath !== previousPath) {
            logger.platform('🔄 Page navigation detected', {
                from: previousPath,
                to: currentPath
            });
            ...
            if (parametersToCleanup.length > 0) {
                ...
                logger.platform('🧹 Cleaned up irrelevant parameters', {
    ...
    const clearPageParams = useCallback(() => {
        ...
        if (Object.keys(clearUpdates).length > 0) {
            ...
            logger.platform('🧹 Cleared all page-specific parameters', { clearedParams: Object.keys(clearUpdates) });
        }
    }, [safeSetQuery, query]);
