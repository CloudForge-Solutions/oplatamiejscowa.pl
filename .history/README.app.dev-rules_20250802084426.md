...
- **WebSocket**: Real-time payment status updates (WebSocketService)
...
IMPORTANT: payment status updates must be real-time via WebSocket with hybrid storage coordination:
...
- **WebSocket**: Real-time updates (ephemeral, cached in memory with localStorage backup)
...
- **Real-time**: WebSocket via WebSocketService for payment status updates
...
**REAL-TIME DATA (WebSocket via WebSocketService):**
- Payment status updates (imoje webhook notifications)
