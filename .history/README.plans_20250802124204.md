...
```
src/
├── app/                       # Static React App (desktop-first payment system)
│   ├── apps/                      # Feature-based applications (tourist/landlord modes)
│   │   ├── tourist-tax/           # Tourist payment flow (desktop-optimized)
│   │   │   ├── TouristTaxPage.tsx        # Main tourist tax page
│   │   │   ├── components/
│   │   │   │   ├── ReservationDisplay.tsx    # Reservation details (desktop-optimized)
│   │   │   │   ├── PaymentForm.tsx           # imoje payment form (desktop methods)
│   │   │   │   ├── PaymentStatus.tsx         # EventBus-driven status
...
│   │   │   │   └── useEventBusUpdates.ts     # EventBus status updates
│   │   │   ├── services/
│   │   │   │   ├── PaymentService.ts         # imoje integration
│   │   │   │   ├── ReservationService.ts     # Reservation data
│   │   │   │   ├── TaxCalculationService.ts  # Tax calculation logic
│   │   │   │   └── CityDataService.ts        # City data management
│   │   │   ├── constants/
│   │   │   │   ├── PaymentConstants.ts       # Payment-related constants
│   │   │   │   └── TaxConstants.ts           # Tax calculation constants
│   │   │   └── types/
│   │   │       ├── PaymentTypes.ts           # Payment interfaces
│   │   │       └── ReservationTypes.ts       # Reservation interfaces
│   │   └── landlord/              # Landlord management interface
│   │       ├── LandlordPage.tsx          # Main landlord page
│   │       ├── components/
│   │       │   ├── PropertyManagement.tsx   # Property management
│   │       │   └── PaymentHistory.tsx       # Payment history view
│   │       └── services/
│   │           └── PropertyService.ts       # Property management logic

├── platform/                  # Core platform services (TypeScript modules)
│   ├── storage/               # Storage abstraction layer
│   │   ├── BlobStorageService.ts     # Azure Blob operations
│   │   ├── LocalStorageManager.ts    # Browser localStorage (repository pattern)
│   │   ├── IndexedDBManager.ts       # IndexedDB operations (large data)
│   │   └── CacheService.ts           # Hybrid caching layer
│   ├── api/                   # API communication
│   │   ├── ApiService.ts             # Backend API calls
│   │   ├── EventBusService.ts        # EventBus coordination
...
│       ├── eventBusUtils.ts          # EventBus utilities
│       └── ServicesManager.ts        # Service dependency injection
...
└── shell/                     # Application shell (desktop-first)
    ├── App.tsx                       # Root application component
    ├── components/
    │   ├── Layout.tsx                    # Main layout component (desktop-optimized)
    │   ├── ErrorBoundary.tsx             # Error boundary wrapper
    │   ├── LoadingSpinner.tsx            # Loading state component
    │   ├── Navigation.tsx                # Desktop navigation
    │   ├── LanguageSwitcher.tsx          # Language switching component
    │   └── ModeSwitcher.tsx              # Tourist/Landlord mode switcher
    ├── context/
    │   ├── ServiceContext.tsx            # Service dependency injection context
    │   └── LanguageContext.tsx           # i18next integration with URL sync
    ├── services/
...
│   ├── StorageConstants.ts           # Storage keys and cache TTL values
│   ├── EventConstants.ts             # EventBus event names and types
│   ├── PaymentConstants.ts           # Payment-related constants
│   ├── UIConstants.ts                # UI states and component constants
│   ├── APIConstants.ts               # API endpoints and error codes
...
#### TouristTaxPage Component
**Responsibility**: Tourist tax payment page presentation
**Single Responsibility**: Coordinate payment flow with EventBus integration and URL sync

```typescript
// src/app/apps/tourist-tax/TouristTaxPage.tsx
import {
  PAYMENT_STATUS,
  UI_STATES,
  EVENT_NAMES,
...
interface TouristTaxPageProps {
  reservationId: string;
}

export const TouristTaxPage: React.FC<TouristTaxPageProps> = ({ reservationId }) => {
  // Single responsibility: Presentation with EventBus coordination
  const { reservation, loading, error } = useReservation(reservationId);
  const { paymentStatus, processPayment } = usePayment(reservationId);
  const { eventBusUpdates } = useEventBusUpdates(reservationId);
  const { selectedCity, setSelectedCity } = useQueryParams();
  const { t } = useLanguage();

  // Fail-fast validation as per dev rules
  if (!reservationId) {
    throw new Error(VALIDATION_RULES.RESERVATION_ID_REQUIRED);
  }

  // Desktop-optimized loading states
  ...
  return (
    <DesktopLayout>
      ...
      <PaymentStatus
        status={paymentStatus}
        eventBusUpdates={eventBusUpdates}
...
**Single Responsibility**: Handle single blob operations with repository pattern integration

```typescript
// src/platform/storage/BlobStorageService.ts
import {
  STORAGE_KEYS,
  API_ENDPOINTS,
  CACHE_TTL,
...
export class BlobStorageService {
  ...
  async getReservation(): Promise<Reservation> {
    // Check localStorage cache first (repository pattern)
    const cached = this.localStorageManager.get(this.config.cacheKey);
    if (cached && this.isCacheValid(cached)) {
      logger.debug(LOGGER_CATEGORIES.STORAGE, 'Using cached reservation data');
      return cached.data;
    }
    ...
    try {
      ...
      const reservation = JSON.parse(content) as Reservation;

      this.validateReservationData(reservation);

      // Cache with TTL in localStorage
      this.localStorageManager.set(this.config.cacheKey, {
        data: reservation,
        timestamp: Date.now(),
      ...
      await this.indexedDBManager.storeReservationDetails(reservation);

      return reservation;
    } catch (error) {
      if (error.statusCode === 404) {
        throw new Error(ERROR_CODES.RESERVATION_NOT_FOUND);
      }
      throw new Error(`${ERROR_CODES.BLOB_FETCH_FAILED}: ${error.message}`);
    }
  }
  ...
  private validateReservationData(data: any): void {
    if (!data || !data.id || !data.totalAmount) {
      throw new Error(ERROR_CODES.INVALID_RESERVATION_DATA);
    }
  }
...
**Single Responsibility**: Create secure, time-limited access tokens for individual blobs

```typescript
// src/api/services/security/BlobSASTokenService.ts
import {
  SAS_TOKEN_CONFIG,
  SECURITY_CONSTANTS,
  AUDIT_EVENTS,
...
export class BlobSASTokenService {
  ...
  async generateBlobSpecificSAS(reservationId: string): Promise<BlobSASResponse> {
    ...
    // SECURE: Time-limited blob-specific permissions only
    const expiresOn = new Date();
    expiresOn.setHours(expiresOn.getHours() + SAS_TOKEN_CONFIG.EXPIRY_HOURS);
    ...
    const sasOptions: BlobGenerateSasUrlOptions = {
      ...
      startsOn: new Date(),
      expiresOn: expiresOn, // Time-limited for security
      contentType: 'application/json',
      cacheControl: `public, max-age=${SAS_TOKEN_CONFIG.CACHE_MAX_AGE}`
    };
