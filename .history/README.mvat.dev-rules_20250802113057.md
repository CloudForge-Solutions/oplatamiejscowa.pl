 ...
 *
 * ARCHITECTURE PRINCIPLE: Nested entity storage with FULL ENTITY IDs ONLY
 * Structure:
 {
   'global:entities:v1.0.0-a7f0': [...],                                    // Global keys (flat)
   'global:currentEntity:v1.0.0-a7f0': entityId,                          // Global keys (flat)
   'entity:01986368-1234-5678-9abc-def012345678': {                       // Entity-nested data (FULL UUID)
     'jpk-config': {...},
     'bank-accounts': [...],
     '...-config' : ...
   }
 }
``` and indexeddb databases in isolated per entity dbs and global dbs with hierarchical colon-separated naming
...
2. EntityContext.switchEntity(entityId) called with STRICT UUID validation (36 characters)
...
5. Entity-specific data loaded synchronously via EntityLocalStorageManager using FULL entity ID:
...
8. Current entity stored in localStorage synchronously using FULL entity ID for persistence
...
12. Error handling for invalid entities (non-UUID format) and storage failures
13. Cache invalidation and refresh mechanisms for optimal performance
14. Database connections use hierarchical naming: mVat:FULL-ENTITY-ID:serviceName
```
...
IMPORTANT: forbidden is any fallback to legacy and any form of data migration as we are in development phase before production, also absurd timeouts instead of proper fixes are forbidden, also absurdly long timeouts are forbidden when we are having all data inside the browser local storage or indexeddb. Migration/cleanup services are FORBIDDEN - use prevention-based architecture with strict validation instead
...
NOTE: magic string only for example purpose, in codebase it should use variables
{
  'global:entities:v1.0.0-a7f0': [...],                                    // Global keys (flat)
  'global:currentEntity:v1.0.0-a7f0': entityId,                          // Global keys (flat)
  'entity:01986368-1234-5678-9abc-def012345678': {                       // Entity-nested data (FULL UUID ONLY)
    'jpk-config': {...},
    'bank-accounts': [...],
    'records-cache': {...}
  }
...
**IndexedDB Database Naming (Hierarchical Colon-Separated):**
```
// Entity Databases (Tab-Based Organization)
mVat:01986368-1234-5678-9abc-def012345678:records    // Records tab
mVat:01986368-1234-5678-9abc-def012345678:jpk       // JPK tab
mVat:01986368-1234-5678-9abc-def012345678:documents // Documents tab
mVat:01986368-1234-5678-9abc-def012345678:audit     // Audit tab
mVat:01986368-1234-5678-9abc-def012345678:storage   // General storage
mVat:01986368-1234-5678-9abc-def012345678:krs       // KRS tab

// Global Databases (Hierarchical)
mVat:global:audit                    // Global audit logs
mVat:global:cache:vat               // VAT cache
mVat:global:cache:krs               // KRS cache
```

**CRITICAL: Entity ID Format Requirements:**
- ONLY full UUID format (36 characters): `01986368-1234-5678-9abc-def012345678`
- NO short entity IDs (8 characters): `01986368` - FORBIDDEN
- NO entity ID truncation with .slice(0, 8) or .substring(0, 8) - FORBIDDEN
- ALL database names use full entity IDs with colon-separated hierarchical naming
- ALL localStorage keys use full entity IDs - no short ID conversions
...
IMPORTANT: **ENTITY ID ARCHITECTURE RULES:**
- ONLY full UUID format (36 characters) allowed: `01986368-1234-5678-9abc-def012345678`
- NO short entity IDs (8 characters): `01986368` - COMPLETELY FORBIDDEN
- NO entity ID truncation: `.slice(0, 8)`, `.substring(0, 8)` - FORBIDDEN
- NO `toShortEntityId()` or `validateShortEntityId()` functions - REMOVED
- ALL database names use full entity IDs with colon-separated hierarchical naming
- ALL localStorage keys use full entity IDs - no conversions or truncations
- ALL logging must use full entity IDs - no privacy truncation in development
- Entity validation requires exactly 36 characters - strict UUID format enforcement
- Database naming: `mVat:01986368-1234-5678-9abc-def012345678:serviceName` (hierarchical)
- Storage keys: `entity:01986368-1234-5678-9abc-def012345678:keyName` (full UUID)
