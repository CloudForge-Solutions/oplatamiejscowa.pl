...
const BookingReservationImport: React.FC<BookingReservationImportProps> = ({
        ...
        try {
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            try {
              ...
              // Calculate tax amount (will be recalculated when city is selected)
              if (reservation.status === 'confirmed' && reservation.numberOfNights > 0) {
                reservation.taxAmount = reservation.numberOfNights * reservation.numberOfPersons * (selectedCity?.taxRatePerNight || cityTaxRate);
              }
  ...
  const handleCitySelect = (city: CityTaxRate) => {
    ...
    // Recalculate tax for all reservations with the new rate
    const updatedReservations = reservations.map(reservation => {
      if (reservation.status === 'confirmed' && reservation.numberOfNights > 0) {
        return {
          ...reservation,
          taxAmount: reservation.numberOfNights * reservation.numberOfPersons * city.taxRatePerNight
        };
      }
      return reservation;
    });
    ...
    logger.info('City selected, tax rates updated', {
      cityCode: city.cityCode,
      taxRate: city.taxRatePerNight,
