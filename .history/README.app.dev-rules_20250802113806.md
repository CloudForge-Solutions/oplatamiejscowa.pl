...
IMPORTANT: Storage keys, and events should be not hardcoded magic strings, but a references to variables in constants and always do barrel import via `src/constants/index.ts`, also we have fixed rules per global storage/indexeddb keys,  ```typescript
...
IMPORTANT: src/constants should have only one file index.ts, as we should finally have modular constants, but in development phase we do barell import of them all here
