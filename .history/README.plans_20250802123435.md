...
## Secure Blob-Specific SAS Token Architecture with Data Retention

**🔒 Security Architecture:**
- ✅ Time-limited SAS tokens with refresh mechanism (SECURITY FIX)
- ✅ Frontend blob storage access with rate limiting
...
2. **Backend** validates UUID with strict sanitization, checks rate limits, generates time-limited SAS token
3. **Frontend** accesses single private blob directly with SAS token (cached in localStorage with TTL)
4. **Payment** processed via imoje, status updates stored in backend
5. **Frontend** polls payment status via API calls (mobile-optimized intervals, NO WebSocket complexity)
...
8. **Token Refresh** automatic refresh before expiration to maintain access
...
- **Security**: Time-limited SAS tokens + rate limiting + input validation + data archiving + localStorage caching
...
**Core Principles:**
- **Mobile-First**: Touch-friendly UI with 44px minimum touch targets consistently applied
- **Security-First**: Time-limited SAS tokens + rate limiting + input validation + localStorage caching
- **Performance**: Direct blob access with smart caching and mobile-optimized API polling ONLY
- **Cost-Effective**: Minimal backend calls with localStorage persistence
- **EventBus-Driven**: Coordinated updates via EventBus pattern (STRICT: no WebSocket complexity)
- **Simple Storage**: localStorage only (no IndexedDB) with flat structure and TTL management
- **Constants-Driven**: ALL strings/keys via barrel exports from src/constants/index.ts (ENFORCED)
- **Single Responsibility**: Each component/service has one clear purpose (STRICT separation)
- **Fail-Fast Validation**: Strict validation with immediate error throwing and sanitization
- **Deployment-Separated**: GitHub Pages (frontend) + Azure Functions (backend)
- **Error Handling**: Consistent error patterns with mobile-optimized user feedback
