...
import { ImojePaymentService, ImojePaymentRequest, ImojeConfig } from '../services/ImojePaymentService';
...
const IMOJE_CONFIG: ImojeConfig = {
  merchantId: process.env.REACT_APP_IMOJE_MERCHANT_ID ?? 'demo-merchant',
  serviceId: process.env.REACT_APP_IMOJE_SERVICE_ID ?? 'demo-service',
  serviceKey: process.env.REACT_APP_IMOJE_SERVICE_KEY ?? 'demo-key',
  environment: (process.env.REACT_APP_IMOJE_ENVIRONMENT as 'sandbox' | 'production') ?? 'sandbox',
  apiUrl: process.env.REACT_APP_IMOJE_API_URL ?? 'https://sandbox.imoje.pl/api/v1',
  jsUrl: process.env.REACT_APP_IMOJE_JS_URL ?? 'https://sandbox.imoje.pl/js/imoje.min.js'
};
...
export const usePaymentProcessing = (): UsePaymentProcessingReturn => {
  ...
  const processPayment = useCallback(async (request: PaymentProcessingRequest): Promise<PaymentProcessingResult> => {
