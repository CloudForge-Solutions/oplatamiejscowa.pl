...
import { Form, Button, Table, Alert, Badge, ProgressBar, Modal, Row, Col, Dropdown, ButtonGroup } from 'react-bootstrap';
  ...
  const importSelected = async () => {
    ...
    try {
      ...
      const reservationsWithCity = selectedReservationData.map(reservation => ({
        ...
        // Set accommodation address to city name since Booking.com export doesn't include property address
  ...
  const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (file && (file.name.endsWith('.xlsx') || file.name.endsWith('.xls'))) {
        ...
        setErrors(prev => [...prev, t('import.invalidFileType', 'Please select an Excel file (.xlsx or .xls)')]);
      }
    }
  }, [t]);
