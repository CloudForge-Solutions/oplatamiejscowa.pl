...
const BookingReservationImport: React.FC<BookingReservationImportProps> = ({
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            try {
              ...
              const reservation: BookingReservation = {
                ...
                guestName: `${row[2] || ''} ${row[3] || ''}`.trim(),
                guestEmail: '', // Not in sample file
                guestPhone: row[17] || '',
                guestCountry: row[11] || 'PL',
                checkInDate: parseBookingDate(row[4]),
                checkOutDate: parseBookingDate(row[5]),
                numberOfPersons: parseInt(row[9]) || 1,
                numberOfNights: 0, // Will be calculated
                accommodationName: row[16] || '',
                accommodationAddress: row[18] || '',
                status: parseBookingStatus(row[7]),
                totalPrice: parseFloat(row[12]) || 0,
                commission: parseFloat(row[14]) || 0,
                paymentStatus: row[15] || 'pending',
                bookingDate: parseBookingDate(row[6]),
  ...
  // Parse Booking.com status
  const parseBookingStatus = (status: string): BookingReservation['status'] => {
  ...
  const importSelected = () => {
    ...
    alert(`Imported ${selectedReservationData.length} reservations successfully!`);
  };
