...
/**
 * BlobStorageService - Azure Storage integration using CDN SDK
 */
export class BlobStorageService {
  ...
  constructor(config: BlobStorageConfig) {
    ...
    try {
      // Wait for Azure Storage SDK to be available from CDN
      if (typeof window !== 'undefined' && window.AzureStorageBlob) {
        this.blobServiceClient = window.AzureStorageBlob.BlobServiceClient.fromConnectionString(config.connectionString);
        this.containerClient = this.blobServiceClient.getContainerClient(config.containerName);

        logger.info('🗄️ BlobStorageService initialized with CDN SDK', {
        ...
        throw new Error('Azure Storage SDK not available. Make sure the CDN script is loaded.');
      }
    } catch (error) {
