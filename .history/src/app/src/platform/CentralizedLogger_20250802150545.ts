// src/platform/CentralizedLogger.ts
// TypeScript implementation of centralized logging system
// ARCHITECTURE COMPLIANCE: Single source of truth for all logging

import type {LoggerInterface} from '@/platform/types/core';
import { DATABASE_NAME_GENERATORS, GLOBAL_DATABASE_SERVICES } from '@/constants';
 ...
 * - Automatic log level detection from message content
 * - File path detection for precise debugging
...
export class CentralizedLogger implements LoggerInterface {
    ...
    private currentLogLevel: string = 'debug'; // Show all logs by default
    private enableConsoleOutput: boolean = true;
    private enablePerformanceTracking: boolean = true;

    // IndexedDB persistence properties
    private indexedDBInitialized: boolean = false;
    private indexedDBInitializing: boolean = false;
    ...
    constructor() {
        // FIXED: Use console directly to avoid circular dependency
        console.info('📝 CentralizedLogger initialized at', new Date().toISOString());

        // Initialize IndexedDB for log persistence (async, non-blocking)
        // CRITICAL FIX: Proper error handling without breaking the logger
        this.initializeIndexedDB().catch(error => {
            console.warn('❌ IndexedDB initialization failed, logs will be queued in memory:', error);
