...
export class EntitySwitchPerformanceTracker {
	...
	startMeasurement(entityId, operation = 'switch') {
		...
		logger.platform('⏱️ Started entity switch measurement', {
			measurementId,
			entityId: entityId,
			operation
		});
	...
	endMeasurement(measurementId) {
		...
		logger.platform(`⏱️ Entity switch completed - ${performanceLevel}`, {
			measurementId,
...
export async function measurePerformance(operation, asyncFunction) {
	...
	try {
		...
		logger.platform(`⏱️ ${operation} completed`, {
			duration: `${duration.toFixed(2)}ms`,
			operation
		});
		...
		logger.platform(`⏱️ ${operation} failed`, {
			duration: `${duration.toFixed(2)}ms`,
			operation,
			error: error.message
		});
