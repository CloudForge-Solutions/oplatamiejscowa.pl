...
const StaysManager: React.FC<StaysManagerProps> = ({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (excelFile) {
      // Show import section and let BookingReservationImport handle the file
      setShowImportSection(true);
      // We'll need to pass the file to the import component
  ...
  return (
    <div>
      ...
      {reservations.length === 0 ? (
        <Alert variant="light" className="mb-4 text-center">
          <div className="mb-3">
            <i className="bi bi-upload display-4 text-muted"></i>
          </div>
          <h5>{t('stays.noStaysYet', 'No stays yet')}</h5>
