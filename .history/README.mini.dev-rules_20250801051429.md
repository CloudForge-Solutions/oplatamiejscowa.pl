...
- **Entity in URL**: Active entity as query parameter
- **Payment preview**: Key-based payment/reservation in route parameter
- **No framework conflicts**: Never fight with Bootstrap, React, Router

## 💾 Storage Architecture

### 🏗️ Entity-Aware Storage Structure
...
{
  'global:entities:v1.0.0': [...],              // Global entities list
  'global:currentEntity:v1.0.0': entityId,     // Current active entity
  'global:language:v1.0.0': 'pl',              // User language preference
  'entity:landlord-123': {                      // Entity-nested data
    'payment-config': {...},                   // Payment gateway config
    'reservations-cache': [...],               // Cached reservations
    'tax-rates': {...}                         // City tax rates
  }
}
```

**IndexedDB Structure**:
- **Per-entity databases**: Isolated data per landlord/entity
- **Global databases**: Shared data (city lists, tax rates, etc.)
- **Large data storage**: Payment receipts, reservation files, audit logs
...
- **Location**: `src/shell/context/ServiceContext.tsx`
- **Hooks**: `useServices()`, `useService(name)`, `useCommonServices()`
- **Services**: PaymentService, ReservationService, TaxCalculationService
...
- **Purpose**: Landlord/entity switching with sub-millisecond performance
- **Location**: `src/shell/context/EntityContext.tsx` (to be implemented)
- **Hook**: `useEntity()`
- **Data**: Payment config, reservations cache, tax rates
- **Performance**: Synchronous localStorage + in-memory caching

### 🎯 Architecture Principles
1. **Separation of Concerns**: Each layer has distinct responsibilities
2. **Automatic Reloading**: Entity data reloads on context changes
3. **Type Safety**: Full TypeScript coverage with strict typing
4. **Event-Driven**: EventBus coordination between contexts
5. **Performance**: Sub-millisecond entity switching
6. **URL Synchronization**: All contexts sync with URL parameters
