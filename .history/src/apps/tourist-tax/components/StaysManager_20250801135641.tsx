...
import { reservationRepository, ReservationStats } from '../repositories/ReservationRepository';
import FileDropZone from './FileDropZone';
import ImportPreviewModal from './ImportPreviewModal';
import StaysListManager, { CityGroup } from './StaysListManager';
import { ImportState } from './ImportStateManager';
...
// Import state management (mVAT-style)
type ImportState =
  | { type: 'idle' }
...
const StaysManager: React.FC<StaysManagerProps> = ({
  ...
  // Import state (mVAT-style inline flow)
  ...
  const processImportFile = async (file: File) => {
    try {
      setImportState({ type: 'processing', file, progress: 20 });

      // Simulate processing with progress updates
      await new Promise(resolve => setTimeout(resolve, 500));
      setImportState(prev => prev.type === 'processing' ? { ...prev, progress: 60 } : prev);

      await new Promise(resolve => setTimeout(resolve, 500));
      setImportState(prev => prev.type === 'processing' ? { ...prev, progress: 100 } : prev);

      // Mock parsed reservations for now
      ...
      setImportState({ type: 'preview', reservations: mockReservations });
    } catch (error) {
  ...
  return (
    <div>
      ...
      {/* Inline Import Preview (mVAT-style) */}
            ...
            <div className="d-flex gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setImportState({ type: 'complete', importedCount: importState.reservations.length });
                }}
              >
      ...
      {importState.type === 'complete' && (
        <Alert variant="success" className="mb-4" dismissible onClose={() => setImportState({ type: 'idle' })}>
